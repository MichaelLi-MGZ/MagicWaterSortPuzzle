...
public class AdsControl : MonoBehaviour, IUnityAdsInitializationListener, IUnityAdsLoadListener, IUnityAdsShowListener
{
    ...
    public ADS_TYPE currentAdsType = ADS_TYPE.ADMOB;
...
#endif
        // Only initialize Unity Ads when using UNITY or MEDIATION ad types
...
#endif
        // Guard against empty/invalid game id
        if (string.IsNullOrEmpty(unityGameId))
        {
            Debug.Log("Skipping Unity Ads initialization: empty gameId");
            return;
        }
